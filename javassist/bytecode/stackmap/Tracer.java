...
public abstract class Tracer implements TypeTag
{
    ...
    protected int doOpcode(final int n, final byte[] array) throws BadBytecode {
        try {
            ...
            throw new BadBytecode("inconsistent stack height " + ex.getMessage(), ex);
        }
    }
    ...
    private void constructorCalled(final TypeData typeData, final int n) {
        ...
        for (int i = 0; i < this.stackTop; ++i) {
            this.stackTypes[i].constructorCalled(n);
        }
        for (int j = 0; j < this.localsTypes.length; ++j) {
            this.localsTypes[j].constructorCalled(n);
        }
    }
    ...
    private void pushMemberType(final String s) {
        ...
        switch (s.charAt(n)) {
            case '[':
                stackTypes[stackTop] = new TypeData.ClassName(s.substring(n));
                break;
            case 'L':
                stackTypes[stackTop] = new TypeData.ClassName(getFieldClassName(s, n));
                break;
            case 'J':

...
public class Executor implements Opcode
{
    ...
    private Type getType(final String s) throws BadBytecode {
        try {
            ...
            throw new BadBytecode("Could not find class [pos = " + this.lastPos + "]: " + s);
        }
    }
    ...
    private Type[] paramTypesFromDesc(final String s) throws BadBytecode {
        ...
        try {
            parameterTypes = Descriptor.getParameterTypes(s, this.classPool);
        }
        catch (NotFoundException ex) {
            throw new BadBytecode("Could not find class in descriptor [pos = " + this.lastPos + "]: " + ex.getMessage());
        }
    ...
    private Type returnTypeFromDesc(final String s) throws BadBytecode {
        CtClass returnType;
        try {
            returnType = Descriptor.getReturnType(s, this.classPool);
        }
        catch (NotFoundException ex) {
            throw new BadBytecode("Could not find class in descriptor [pos = " + this.lastPos + "]: " + ex.getMessage());
        }
